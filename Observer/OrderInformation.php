        ...
        if ($area != "adminhtml") {
            ...
            if (true) {
                ...
                if ($resx["cya_code"] == 401) {
                    $order->addStatusHistoryComment("(ValidAge) ERROR: This order was not validated");
                    $order->save();
                    $this->orderManagement->hold($order->getEntityId());
                }
                if ($resx["cya_code"] == 201) {
                    $order->addStatusHistoryComment("(ValidAge) WARNING: " . $resx["cya_message"]);
                    $order->save();
                }
                if ($resx["cya_code"] == 200) {
                    $order->addStatusHistoryComment("(ValidAge) APPROVED!! ");
                    $order->save();
                }
            } else {
                exit;
